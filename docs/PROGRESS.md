...
**当前阶段：13 基础功能完善（浏览体验，已完成，详见 [`13-浏览体验增强.md`](./13-浏览体验增强.md)）**
> 文章列表支持关键词搜索（标题/正文）、难度筛选、排序、加载更多分页。
> 下一步：学习记录（已读/进度/收藏/统计）→ 用户体系（登录+个人后台+数据云端化）→ 可切换主题。
...
| 13 | 基础功能完善（搜索、难度筛选、排序、加载更多） | [`13-浏览体验增强.md`](./13-浏览体验增强.md) | ✅ |
| 14 | 复盘（全流程回顾、博客素材整理） | `14-复盘.md`（待建） | ⬜ |
...
- **日期**：2026-08-04
- **完成内容**：后端 `GET /api/articles` 加 `q`（标题/摘要/正文 LIKE 搜索）、`difficulty` 过滤；前端 `fetchArticles(query)` 统一过滤（本地文章同规则）+ 合并后排序（latest/longest/shortest）；列表页加搜索框（`useDeferredValue` 防抖）、难度 tabs、排序下拉、加载更多（每批 10 篇）
- **决策记录**：正文搜索放后端（列表接口无 content），本地文章前端补过滤；当前量级用前端分批渲染代替后端分页；排序在合并后统一执行保证顺序一致
