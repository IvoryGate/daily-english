...
**当前阶段：12 体验增强（定时抓取 + 抓取异步化 + 真 FSRS，已完成，详见 [`12-体验增强.md`](./12-体验增强.md)）**
> 爬虫支持每天 08:00 定时增量抓取；抓取任务异步化（后台线程 + 轮询）；闪卡升级为 FSRS 算法（四档评价）。
> 下一阶段：完善功能、打造主题网站、优化样式。
...
| 13 | 复盘（全流程回顾、博客素材整理） | `13-复盘.md`（待建） | ⬜ |
