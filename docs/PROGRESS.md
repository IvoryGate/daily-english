...
**当前阶段：15 用户体系（进行中：15.1 后端认证基础 ✅ / 15.2 数据云端化 ✅ / 15.3 前端认证 ✅ / 15.4 数据切换+迁移 ✅ / 15.5 个人后台）**
> 15.1 完成：`User` 模型 + bcrypt 密码哈希 + PyJWT 认证 + 注册/登录/me 接口 + `get_current_user` 依赖。
> 15.2 完成：生词/收藏/阅读记录三张表（`vocabulary_entries`/`bookmarks`/`reading_records`，均带 `user_id` 外键）+ `/api/me/*` 全套 CRUD + 登录后一次拉全量的 `GET /api/me/data`。
> 15.3 完成：前端 AuthContext + 登录/注册页 + ProtectedRoute 路由守卫 + SiteHeader 登录态（用户名/退出）。
> 15.4 完成：`UserDataContext` 数据层——登录走云端 `/api/me/*`、未登录走 localStorage；登录时自动把本地旧数据一次性迁移到云端（生词带 FSRS 卡、阅读带时间戳）；ArticleCard/ArticleReader/详情页/统计/生词本/复习页全部接入。
> 下一步：15.5 个人后台（账号信息、数据管理）。
...
| 15 | 用户体系（登录认证、数据云端化、个人后台） | [`15-用户体系.md`](./15-用户体系.md) | 🚧 15.4 ✅，进行中 |
| 16 | 复盘（全流程回顾、博客素材整理） | `16-复盘.md`（待建） | ⬜ |
