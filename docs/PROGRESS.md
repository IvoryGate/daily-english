...
**当前阶段：11 爬虫架构演进（可插拔来源 + 风控，已完成，详见 [`11-爬虫架构演进.md`](./11-爬虫架构演进.md)）**
> 爬虫重构为可插拔 provider 架构（VOA/卫报/大西洋月刊），每源接入限速+指数退避+熔断风控。
> 注意：卫报与大西洋月刊全文均有版权，当前仅个人学习用，风险已由用户确认接受。
...
| 12 | 复盘（全流程回顾、博客素材整理） | `12-复盘.md`（待建） | ⬜ |
