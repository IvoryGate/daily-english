...
> 阶段 20 完成（AI 深度集成）：默认用 OpenCode Zen 免费模型（匿名无需 key，安装即用），用户可 BYOK；后端 OpenAI 兼容代理 + 工具循环（联网搜索/查词/读文章/读统计/存笔记）+ 流式 SSE；前端浮动聊天面板 + 生词本/复习页 AI 讲解入口，见 [`20-AI深度集成.md`](./20-AI深度集成.md)。
