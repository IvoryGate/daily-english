...
**阶段 01~19 全部完成 ✅（规划 → 用户体系 → 复盘 → 视觉优化 → 学习体系 → 信息架构重排）**
> 阶段 19 完成（信息架构重排）：单站分路由——前台 `/`（文章/阅读）+ 个人中心 `/account/*`（我的学习/生词本/复习/设置/添加文章，二级导航）；导航从 8 项精简到 3-4 项；Dashboard 拆组件（633→516 行，图表抽到 charts.tsx）；旧路径全部重定向，见 [`19-信息架构重排.md`](./19-信息架构重排.md)。
