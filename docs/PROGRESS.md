...
**阶段 01~17 全部完成 ✅（规划 → 用户体系 → 复盘 → 视觉打磨与功能优化）**
> 阶段 17 完成（视觉 + 功能）：温暖纸感配色（米白底/暖墨字/陶土棕主色）+ 阅读衬线排版 + 进度条；功能侧加了深色模式、字号调节、朗读全文、生词发音、导出 Anki、乱序复习、随机一篇、今日推荐，见 [`17-视觉打磨与功能优化.md`](./17-视觉打磨与功能优化.md)。
> 词典升级：弃用纯英英 dictionaryapi.dev，后端代理有道词典（中文释义 + 美英音标 + 真人发音音频）。
> 阶段 16 复盘 + 修复：`requirements.txt` 补齐到实际使用的 10 个依赖。
> 待办（见复盘"下一步建议"）：写 README（本轮完成后写）、真实部署（等服务器/域名）、SECRET_KEY 环境变量化、卫报/大西洋版权合规、补最小测试集。
