...
**当前阶段：15 用户体系 ✅ 完成（15.1 后端认证 ✅ / 15.2 数据云端化 ✅ / 15.3 前端认证 ✅ / 15.4 数据切换+迁移 ✅ / 15.5 个人后台 ✅）**
...
> 15.4 完成：`UserDataContext` 数据层——登录走云端 `/api/me/*`、未登录走 localStorage；登录时自动把本地旧数据一次性迁移到云端（生词带 FSRS 卡、阅读带时间戳）。
> 15.5 完成：`/settings` 个人后台——改用户名/改密码 + 导出 JSON 备份 + 一键清空数据（二次确认）。
> 下一步：阶段 16 复盘。
